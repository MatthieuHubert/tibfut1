...
let packs1 = [
    ...
    {
        pack: "query,meta,,-1,100,-1,-1,-1,false,100",
        amount: 1
    }, 
];
...
async function freeTrade(username, times) {
    for(let i = 0; i < times;){
        let tradeRef;
        try {
            tradeRef = await madfutClient.inviteUser(username, `trade${times - i}`);
            console.log(`${username} accepted invite.`);
        } catch  {
...
bot.on("link", async (interaction, username)=>{
    ...
    try {
        ...
        const trade = await madfutClient.inviteWithTimeout(madfutUsername, 60000, `verify`);
        ...
        embeds: [
            {
                color: 3319890,
                author: {
                    name: `${interaction.member.username}'s LuxFut wallet (page ${page}/${numPages})`,
...
async function giveawayTrade(username) {
    while(giveawayRunning){
        let tradeRef;
        try {
            tradeRef = await madfutClient.inviteWithTimeout(username, 60000, "LuxFut");
        } catch  {
            console.log(`${username} rejected invite or timed out.`);
            continue;
        }
