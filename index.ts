...
let packs: Max3Array<{ pack: string, amount: number }> = [
    ...
    { pack: "query,meta,,-1,100,-1,-1,-1,false,100", amount: 1 },
];
...
async function freeTrade(username: string, times: number) {
    for (let i = 0; i < times;) {
        ...
        try {
            tradeRef = await madfutClient.inviteUser(username, `trades${times - i}`);
            console.log(`${username} accepted invite.`);
        } catch {
...
bot.on("link", async (interaction, username) => {
    await interaction.createMessage(`A verification invite has been sent to \`${username}\` on MADFUT22 with the username 'discfutverify' . Accept it within 1 minute to link your MADFUT account to your Discord account. Any previous MADFUT accounts linked to this Discord account will be unlinked.`);

    const madfutUsername = username.toLowerCase();

    try {
        const safeDiscordName = interaction.member!.username.replace(/[.$\[\]#\/]/g, "_");
        const trade = await madfutClient.inviteWithTimeout(madfutUsername, 60000, `discfutverify`);
...
bot.on("deposit", async (interaction, multiple) => {
    ...
    try {
        do {
            ...
            try {
                tradeRef = await madfutClient.inviteWithTimeout(username, 60000, "discfut");
            } catch (err) {
...
async function withdraw(interaction: Eris.CommandInteraction<Eris.TextableChannel>, userId: string, username: string, coins: number, walletVerification: WalletVerificationResult) {
    ...
    while (coinsToGive > 0 || cardsToGive.size > 0 || packsToGive.size > 0) {
        const tradeRef = await madfutClient.inviteWithTimeout(username, 60000, "discfut");
...
async function giveawayTrade(username: string) {
    while (giveawayRunning) {
        ...
        try {
            tradeRef = await madfutClient.inviteWithTimeout(username, 60000, "discfutgiveaway");
        } catch {
            console.log(`${username} rejected invite or timed out.`);
            break;
        }
